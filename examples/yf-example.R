...
##
arima_model = list(p = 1, d = 1, q = 0, P = 1, D = 0, Q = 0) # NULL

##
## Optional co-variate for SARIMA. Used ONLY if 'arima_model' is defined by the User and method = 'stat'
##

covar = 'precip' #'precip' # or 'temp' or 'precip' default is FALSE
...
disease ='yellow_fever' #'dengue' #'flu'
...
output <- runDICE(data_source=data_source,year=year, nreal=nreal, nMCMC = nMCMC, mod_level = mod_level, fit_level = fit_level, RegState = RegState,model= model, nfit = nfit, isingle = isingle, subDir = subDir, device = 'pdf', plot = plot, Tg = Tg, prior = prior, Temp = Temp, da = da, epi_model=epi_model, db_opts=db_opts, disease = disease, method = method, arima_model = arima_model, covar = covar, covar_lag = covar_lag)



##out <- runDICE(data_source = data_source, year=year, nreal=nreal, nMCMC = nMCMC, mod_level = mod_level, fit_level = fit_level, RegState = RegState, model= model, nfit = nfit, isingle = isingle, subDir = subDir, Tg = Tg, Temp = Temp, da = da, epi_model=epi_model, db_opts=db_opts, disease = disease, method = method, arima_model = arima_model, covar = covar, covar_lag = covar_lag)
