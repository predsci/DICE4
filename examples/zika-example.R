...
if(length(args) == 0) {
  ...
  CDC_server    Logical determining if CDC data should be retrieved from the CDC server
    ...
    if ('mod_name'    %in% names(args))  mod_name = c(NAME_2=args$mod_name)
 }
...
output <- runDICE(data_source=data_source,year=year, nreal=nreal, nMCMC = nMCMC, mod_level = mod_level, fit_level = fit_level, RegState = RegState,mod_name = mod_name, fit_names = fit_names, model= model, nfit = nfit, isingle = isingle, subDir = subDir, device = 'pdf', plot = plot, Tg = Tg, prior = prior, Temp = Temp, da = da, epi_model=epi_model, db_opts=db_opts, disease = disease, method = method, arima_model = arima_model, covar = covar, covar_lag = covar_lag)
