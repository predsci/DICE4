...
##
arima_model = list(p = 0, d = 1, q = 1, P = 0, D = 1, Q = 0) # NULL
...
##
## Spatial level of model/fit data and names
##

mod_name=c(NAME_1="Europe")

mod_level = 1

fit_names=c("BE", "CH", "CZ", "DK", "ES", "FI", "FR", "GR", "HR", "HU", "IE", "IT", "LT", "NO", "PL", "RS", "SK")
...
## 1 Use historic weekly/monthly average, NULL model
...
out <- runDICE(data_source = data_source, year=year, nreal=nreal, nMCMC = nMCMC, mod_level = mod_level, fit_level = fit_level, mod_name = mod_name, fit_names = fit_names, model= model, nfit = nfit, isingle = isingle, subDir = subDir, Tg = Tg, Temp = Temp, da = da, epi_model=epi_model, db_opts=db_opts, disease = disease, method = method, arima_model = arima_model, covar = covar, covar_lag = covar_lag, device = device)
